...
//        SevenSeg test = new SevenSeg(List.of("acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab".split(" ")));
//        SevenSeg sevenSegSolution = test.performSearch();
//
//        System.out.println(sevenSegSolution);
//
//        for (String raw : "cdfeb fcadb cdfeb cdbaf".split(" ")) {
//            System.out.println(sevenSegSolution.decypher(raw));
//        }

        List<Day8Input> parsedInputs = parseInput(realInput);
        List<Integer> lengthsOfInputs = SevenSeg.digitToSegments.entrySet().stream().map(kv -> kv.getValue().size()).toList();
        List<Integer> uniqueInputLengths = lengthsOfInputs.stream().filter(x -> {
            return lengthsOfInputs.stream().filter(y -> x == y).count() == 1;
        }).toList();

        int count = 0;

        for (Day8Input inputLine : parsedInputs) {
            for (String encodedOutput : inputLine.encodedOutputs) {
                if (uniqueInputLengths.contains(encodedOutput.length())) {
                    count++;
                }
            }
        }
...
//        Day8Input part2Test = new Day8Input("acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf");
//        SevenSeg part2Key = new SevenSeg(part2Test.calibrations).performSearch();
//        System.out.println("Part 2 Test : " + Arrays.toString(part2Test.encodedOutputs.stream().map(encoded -> part2Key.decypher(encoded)).toArray()));

        int sumTotal = 0;

        for (Day8Input input : parsedInputs) {
            SevenSeg key = new SevenSeg(input.calibrations).performSearch();
            StringBuilder sb = new StringBuilder();
            input.encodedOutputs.forEach(encodedOutput -> {
                sb.append(key.decypher(encodedOutput));
            });
...
//            System.out.println(output);
        ...
        System.out.println("Answer Part 2: " + sumTotal);
    }

    static List<Day8Input> parseInput(String input) {
        Scanner scanner = new Scanner(input);
        List<Day8Input> parsedInputs = new ArrayList<>();
        while (scanner.hasNext()) {
            String line = scanner.nextLine();
            parsedInputs.add(new Day8Input(line.trim()));
        }
        return parsedInputs;
    }
